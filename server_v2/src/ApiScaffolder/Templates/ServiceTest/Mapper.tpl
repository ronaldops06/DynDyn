...
{
    public class {{model}}Mapper : BaseTestService
    {
        ...
        public void Eh_Possivel_Mapear_Os_Modelos()
        {
            ...
            //Entity -> Model
            var {{alias}}Model = Mapper.Map<{{model}}Entity>(entity);
            Assert.Equal({{alias}}Model.Id, entity.Id);
            //
            Assert.Equal({{alias}}Model.UserId, entity.UserId);
            Assert.Equal({{alias}}Model.User.Id, entity.User.Id);
        }
        ...
        public void Eh_Possivel_Mapear_Os_Modelos_Em_Lista()
        {
            ...
            var listEntity = new List<{{model}}Entity>();
            for (int i = 1; i <= 5; i++)
            {
            ...
            var listModel = Mapper.Map<List<{{model}}Model>>(listEntity);
