...
public class WhenExecuteCreate : TransientUserTest
{
    ...
    public async Task Eh_Possivel_Executar_Metodo_Create()
    {
        ...
        var service = new Mock<TransientUserService>(RepositoryMock.Object, UserServiceMock.Object,
            LoginServiceMock.Object, CategoryServiceMock.Object, OperationServiceMock.Object,
            OperationRoleServiceMock.Object, Mapper);
        service.CallBase = true;
        service.Protected()
            .Setup<Task>("SendMailVerification", ItExpr.IsAny<string>(), ItExpr.IsAny<int?>(), ItExpr.IsAny<string>())
